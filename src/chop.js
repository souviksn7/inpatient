...
import { getFhirMeds } from "./controller/medicationController.js";
async function execBigModule(condition) {
  if (condition) {
    try {
  const { getVisitsData } = await import('./controller/visitController.js');
  getVisitsData()
...
//taposh comment
// import democonfig from "../conf/democonfig.js";
...
function render() {
  ...
  encounters.forEach(function (v) {
    ...
    try {
      ...
      chartConfig.rows[rowMap[v.row]].count++;
    ...
    deferreds.push(
      search(
        ...
        try {
          ...
          // Pregrouper-filter immediately to prep for encounter linking.
          preFilterMedications(meds.MedicationOrders, grouper.row);
        } catch (error) {
          chart.failure = true;
          log(error.stack, "error");
...
function fhirMedCallback(meds, state, xhr) {
  try {
    ...
    // console.log("this is fhirmeds",fhirMeds)
  } catch (error) {
    chart.failure = true;
    log(error.stack, "error");
  }
}
// Taposh - this is a repeated code this comes under preFilterEncounters
...
// Taposh - this is repeated code comes under preFilterEncounters
 ...
 **************** Encounter Functions *****************
 ******************************************************/
// this comment is to test the visits
...
//         // console.log([1, 5].indexOf(resource.adtClass))
//         // console.log(resource)
...
//   console.log("this is encounters: ", encounters);
}
...
function  preFilterMedications(medications, row) {
  ...
  // console.log("this is medication", medications)
  ...
  // console.log("this is medId",medIdMap)
}
...
function resizeHealthChart() {
  try {
    ...
    setTimeout(function () {
      windowWidth = window.innerWidth;
      try {
        timeline.options.chart.width =
          windowWidth - windowPadding > 1200
            ? 1200
        ...
        // Limit div to the size of the chart to eliminate EHR scroll bars
        jQuery("#" + chartConfig.namespace).css(
          "width",
          timeline.options.chart.width
        );
        timeline.resize();
...
function visits(enc1) {
  enc1.forEach(function (enc) {
  ...
  encounters = encounters.filter(function (resource) {
    ...
    if ([1, 5].indexOf(resource.adtClass) >= 0) {
      ...
      if (resource.adtClass == 4) {
        encMap[resource.id].row = resource.row = "Allergy";
        resource.shape =
          chartConfig.rows[rowMap[resource.row]].legend.base.shape;
      }
